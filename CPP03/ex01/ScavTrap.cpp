...
/*   Updated: 2025/05/05 10:57:43 by nezumickey       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ScavTrap::attack(const std::string& target)
{
	if (energyPoints && hitPoints)
	{
		...
		<< " uses its signature move on " << RED << target << RESET << std::endl;
	}
	else if (!energyPoints)
		std::cout << BLUE << name << RESET << " doesn't have enough energy." << std::endl;
	else
...
void 			ScavTrap::guardGate()
{
	...
	<< YELLOW << " Guarding gate " << RESET << "ativate" << std::endl;
}
